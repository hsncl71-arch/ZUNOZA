...
function withSecurity(headers: Headers) {
  ...
  if (!headers.has("Content-Security-Policy")) {
    headers.set(
      ...
      "upgrade-insecure-requests; frame-ancestors 'self' https://*.grok.me https://*.grok-sandbox.com https://grok.com https://*.grok.com https://x.com https://*.x.ai; base-uri 'self'; object-src 'none'",
    );
  }
  return headers;
}
...
export default async function grokPwaMiddleware(
  ...
  if (path === "/__grok/manifest.webmanifest" || path === "/__grok/manifest.json") {
    return new Response(
      (renderWebManifest as (host: string, site?: object) => string)(
        requestHost(event),
        grokOgIdentity.site,
      ),
      {
