...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", site: {}, cwd: mkdtempSync(join(tmpdir(), "grok-og-iso-")) });
...
test("document title entities are not double-escaped on og:title", () => {
  const out = injectGrokPwaHead("<html><head><title>Cats &amp; Dogs</title></head></html>", {
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-ent-")),
  });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({
    appName: "Wild Race",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-head-")),
  });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Wild Race",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-pwa-")),
  });
  assert.match(out, /apple-mobile-web-app-title" content="Wild Race"/);
});

test("PWA title uses site short_name", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "lunar-sapphire-honey-able.grok.me",
    site: { title: "ZUNOZA AI VIDEO", short_name: "ZUNOZA" },
  });
  assert.match(out, /apple-mobile-web-app-title" content="ZUNOZA"/);
  assert.match(out, /href="\/__grok\/icon-180.png"/);
});
...
test("streaming injector handles </head> split across chunks", () => {
  const injector = createHeadInjector({
    appName: "Wild Race",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-split-")),
  });
  const chunks = [
    ...injector.push("<html><head><title>x</title></he"),
    ...injector.push("ad><body>hello</body></html>"),
...
test("site short_name wins for PWA home screen name", () => {
  const manifest = JSON.parse(
    renderWebManifest("lunar-sapphire-honey-able.grok.me", { title: "ZUNOZA AI VIDEO", short_name: "ZUNOZA" }),
  );
  assert.equal(manifest.name, "ZUNOZA");
  assert.equal(manifest.short_name, "ZUNOZA");
  assert.equal(manifest.theme_color, "#05050a");
  assert.equal(manifest.background_color, "#05050a");
});

test("Vercel system host still names the PWA ZUNOZA", () => {
  const manifest = JSON.parse(renderWebManifest("app-abc.vercel.app", { title: "ZUNOZA AI VIDEO" }));
  assert.equal(manifest.name, "ZUNOZA AI VIDEO");
  const empty = JSON.parse(renderWebManifest("app-abc.vercel.app", {}));
  assert.equal(empty.name, "ZUNOZA");
});
