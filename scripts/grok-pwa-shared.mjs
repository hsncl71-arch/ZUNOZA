...
export const PRODUCT_PWA_NAME = "ZUNOZA";
...
/** Home-screen name. Product short_name beats the Grok project slug. */
export function resolvePwaName(site = {}, host = "", fallback = PRODUCT_PWA_NAME) {
  const short = String(site.short_name ?? "").trim();
  if (short) return short.slice(0, 24);
  ...
  return fromFallback || PRODUCT_PWA_NAME;
}
...
/**
 * Home-screen / PWA display name. `short_name` wins over share-card title.
 * @param {string} [hostHeader]
 * @param {object} [site]
 * @param {string} [fallback]
 */
export function pwaAppName(hostHeader, site = {}, fallback = "") {
  const short = String(site.short_name ?? "").trim();
...
export function renderWebManifest(hostHeader, site = {}) {
  const name = pwaAppName(hostHeader, site || {});
...
export function grokPwaHeadTags(appName = PRODUCT_PWA_NAME) {
  return [
    ...
    ["theme-color", '<meta name="theme-color" content="#05050a">'],
  ];
}
...
export function injectGrokPwaHead(html, ctx = {}) {
  ...
  const homeScreenName = pwaAppName(host, site, ctx.appName);
  let next = stripShareMetaTags(html);

  const missing = grokPwaHeadTags(homeScreenName)
    .filter(([key]) => {
  ...
  next = insertAfterHeadOpen(
    next,
    grokOgHeadTags({ host, appName: ogTitle, site, documentTitle, cwd }).join(""),
  );
