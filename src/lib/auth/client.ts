...
import { GROK_PROVIDERS, SIGN_IN_PROVIDERS } from "./providers";
...
export { GROK_PROVIDERS, SIGN_IN_PROVIDERS };
...
export async function signIn(
  ...
  if (inLivePreview()) {
    if (!popup) throw new Error("Açılır pencere engellendi. Giriş için pop-up izni verin.");
    const token = await waitForPopupToken(popup);
    if (!token) throw new Error("Giriş iptal edildi veya tamamlanamadı.");
...
export async function signOut(redirectTo = "/"): Promise<void> {
  await runSignOut({
    ...
    requestSignOut: async () => {
      ...
      if (error) throw new Error(error.message ?? "Çıkış yapılamadı.");
    },
    clearToken: () => setBearerToken(null),
    redirect: () => {
