...
import { appleGenericOAuthConfig } from "./apple";
import { pgliteDialect } from "./pglite-dialect";import {
  GROK_ISSUER_DEFAULT,
...
// Kick PGLite bootstrap in the sandbox preview only. Deployed Vercel *builds*
// have GROK_PROJECT_ID without DATABASE_URL; calling ensureDbReady() there used
// to reject at import and abort the production deploy.
if (!process.env.GROK_PROJECT_ID?.trim() && process.env.VERCEL !== "1") {
  void ensureDbReady();
}
...
const ZUNOZA_CUSTOM_ORIGINS: string[] = [
  "https://zunoza.com",
  "https://www.zunoza.com",
  "https://app.zunoza.com",
];
const ZUNOZA_CUSTOM_ORIGINS: string[] = [
  "https://zunoza.com",
  "https://www.zunoza.com",
...
const trustedOrigins: string[] = explicitBaseURL
  ? [explicitBaseURL, ...ZUNOZA_CUSTOM_ORIGINS, ...LOCAL_DEV_ORIGINS]
  : [
      // Host wildcards (matched against Origin's host)
      ...previewAllowedHosts,
      // Full-origin wildcards (matched against Origin)
      ...previewAllowedHosts.flatMap((host) => [`https://${host}`, `http://${host}`]),
      ...ZUNOZA_CUSTOM_ORIGINS,
      ...LOCAL_DEV_ORIGINS,
    ];
...
const appleOAuth = appleGenericOAuthConfig();
...
export const auth = betterAuth({
  ...
  // Encrypt broker-issued OAuth tokens at rest. Google/Apple are trusted
  // identities, but an unverified email/password signup must NOT absorb a later
  // Google/Apple login (account takeover). X first login creates a new user;
  // it does not attach to an unverified local mailbox.
  account: {
  ...
  advanced: {
    ...
    cookies: {
      ...
      // Apple form_post is a cross-site POST, so Lax state/PKCE cookies are
      // dropped. Session cookies stay Lax; only the short-lived OAuth state
      // cookies are SameSite=None; Secure.
      oauth_state: {
        name: "__Host-grok-auth.oauth_state",
        attributes: { sameSite: "none", secure: true, path: "/", httpOnly: true },
      },
      state: {
        name: "__Host-grok-auth.state",
        attributes: { sameSite: "none", secure: true, path: "/", httpOnly: true },
      },
    },
  },
