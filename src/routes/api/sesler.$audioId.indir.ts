...
export const Route = createFileRoute("/api/sesler/$audioId/indir")({
  server: {
    handlers: {
      GET: async ({ params, request }) => {
        ...
        const rows = await sql<{ audio_url: string | null; mime_type: string | null }>`
          select audio_url, mime_type from voice_assets
          where id = ${params.audioId} and user_id = ${user.id}
        `;
        ...
        if (isR2ObjectRef(url)) {
          ...
          if (!obj.ok) return new Response("Ses alınamadı", { status: 502 });
          const mime = obj.contentType || rows[0]?.mime_type || "audio/mpeg";
          return new Response(obj.bytes, {
            headers: {
              "Content-Type": mime,
              "Content-Disposition": `inline; filename="zunoza-ses-${params.audioId}.mp3"`,
        ...
        if (!media?.ok || !media.body) return new Response("Ses alınamadı", { status: 502 });
